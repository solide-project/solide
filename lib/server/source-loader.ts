...
class SolideContractLoader {
    ...
    async generateSource(): Promise<any | string> {
        ...
        const sourceName = raw.replace(/https:\/\/raw.githubusercontent.com\/[a-zA-Z0-9\-]+\/[a-zA-Z0-9\-]+\/[a-zA-Z0-9\-]+\//, "");
...
const flattenContracts = ({
    ...
    dependencies.reverse().forEach((dependency) => {
        ...
        const cleanedContent = removeContractHeaders(fileContents);
        flattenContract += cleanedContent;
    });

    if (content) {
        flattenContract = flattenContract.concat(removeContractHeaders(content))
    }
...
async function extractImports(content: any, mainPath: any = "", libraries: string[] = []): Promise<ContractDependency[]> {
    ...
    while ((match = regex.exec(content)) !== null) {
        ...
        const { fileContents } = await resolve(contractPath.filePath, contractPath.isRelative);
        ...
        matches.push(...(await extractImports(fileContents, contractPath.filePath, libraries)));
    }
 ...
 */
const removeContractHeaders = (content: string): string => {
    if (!content) return "";
 ...
 * @returns 
 */
async function resolve(importPath: string, isRelative: boolean = false): Promise<{ fileContents: string }> {
