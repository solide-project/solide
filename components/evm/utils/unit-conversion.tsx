...
export default function UnitConversion({ }: UnitConversionProps) {
  ...
  useEffect(() => {
    ...
    const matchingUnits = unitEntries.filter(
      ([key, value]) => value === BigInt(`1${"0".repeat(18 - d)}`)
    )
    setUnit(
      matchingUnits.length > 0 ? matchingUnits.map((i) => i[0]).join(" ") : ""
    )
  }, [decimals])
  ...
  return (
    <>
      ...
      <Input
        ...
        placeholder="value"
      />
      <Button className="my-2" size="sm" onClick={handleConvert}>
        Decode
      </Button>
